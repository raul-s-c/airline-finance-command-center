...
def aggregate_t100_rows(
    ...
    *,
    ...
    """Aggregate T-100 while retaining useful route rankings for every network scope.

    Version 2 ranked routes across the entire network before the UI filtered by scope.
    Large domestic routes could therefore occupy every retained slot and make the
    international route view appear empty even though T-100-I data existed. Version 3
    retains the overall leaders plus the top routes inside each scope.
    """
    ...
    scoped_keys: list[tuple[str, str, str]] = []
    scopes = sorted({key[2] for key in route_totals})
    for scope in scopes:
        scope_ranked = [
            key
            for key, _ in sorted(
                ((k, v) for k, v in route_totals.items() if k[2] == scope),
                key=lambda item: number(item[1].get("passengers")),
                reverse=True,
            )[:top_route_count]
        ]
        scoped_keys.extend(scope_ranked)

    top_route_keys = list(dict.fromkeys([*overall_keys, *scoped_keys]))
    ...
    route_scope_counts = {
        scope: sum(1 for row in top_routes if row.get("scope") == scope)
        for scope in scopes
    }
    ...
    return {
        ...
        "route_scope_counts": route_scope_counts,
...
def build_delta_bts_summary_v3(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="afcc-build-bts-v3",
        description="Build Delta BTS outputs with scope-safe route rankings.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = build_delta_bts_summary_v3(args.output, work_dir=args.work_dir, timeout=args.timeout)
    counts = summary["t100_network"].get("route_scope_counts", {})
    print(
        f"Wrote {args.output}: "
        f"{len(summary['p12_financials'])} financial quarters, "
        f"{len(summary['t100_network']['history'])} network months, "
        f"routes by scope={counts}"
    )
    return 0
